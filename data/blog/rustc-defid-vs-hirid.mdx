...
You don't need to write a compiler pass to observe these IDs. `rustc` has unstable flags that dump the intermediate representations directly to stdout.

**Print the HIR tree with HirIds annotated:**

```bash
rustc -Z unpretty=hir,identified your_file.rs
```

The `,identified` modifier adds the `HirId` of each node inline. Output looks like this:

```rust
fn main() /* HirId(DefId(0:3 ~ demo[XXXX]::main).0) */ {
    let x /* HirId(...) */ = 1 /* HirId(...) */ + 2 /* HirId(...) */;
}
```

Every node — the function, the let binding, the literals, the binary expression — has its own `HirId`.

**Print the raw HIR tree (no source reconstruction):**

```bash
rustc -Z unpretty=hir-tree your_file.rs
```

This prints the actual Rust structs (`Expr`, `Stmt`, `Block`, etc.) with all fields, so you can see exactly what the HIR looks like before type-checking.

**Print MIR (which uses DefIds):**

```bash
rustc -Z unpretty=mir your_file.rs
# or dump all MIR bodies to files:
rustc -Z dump-mir=all your_file.rs
```

MIR output labels each body with its `DefId` path, e.g. `demo::main`. You'll also see `DefId`s in calls and type references within the body.

**Print DefId paths from inside a compiler pass:**

```rust
// The human-readable path for any DefId
let path = tcx.def_path_str(def_id);  // e.g. "std::vec::Vec"

// Debug-print a HirId directly
eprintln!("{:?}", expr.hir_id);        // e.g. HirId(DefId(0:3 ~ demo[a1b2]::main).4)
```

`def_path_str` is the most useful one when debugging — it turns an opaque `DefId` into the qualified path you'd write in source code.
